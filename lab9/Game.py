...
class Coin(pygame.sprite.Sprite):   # Define Coin as a sprite (inherits from pygame's Sprite)
    def __init__(self):     # runs when a Coin object is created
        super().__init__()      # Initialize parent Sprite class  // allows the child to inherit and extend the parent's initialization logic.
...
while True:
    ...
    scores = font_small.render(str(SCORE), True, BLACK)      # Render the current score 
    coin_scores = font_small.render(f"Coins: {COIN_SCORE}", True, BLACK)    # Render the coin count 
    DISPLAYSURF.blit(scores, (10, 10))      # Draw the score text near the top-left corner
